...
static int imx_ssi_set_dai_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_I2S:
		...
		printk("  SSI Working in I2S mode  %s(%d)  %s \n",__FILE__,__LINE__,__func__);
		break;
...
static int imx_ssi_dev_probe(struct platform_device *pdev)
{
	...
	priv->ioaddr = ioremap(res->start, 0x5C);
        if( priv->ioaddr) 
//	printk("  ssi ioremap  OK! address=%x,    %s(%d)  %s \n",priv->ioaddr, __FILE__,__LINE__,__func__);
	...
	for (fifo = 0; fifo < 2; fifo++) {
		...
		imx_ssi_dai[channel] = dai;
                printk("imx_ssi_dai[%d]   dai->id=%d     %s(%d)  %s \n",channel,channel, __FILE__,__LINE__,__func__);

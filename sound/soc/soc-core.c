...
static void snd_soc_instantiate_card(struct snd_soc_card *card)
{
	...
	list_for_each_entry(platform, &platform_list, list)
		if (card->platform == platform) {    // imx_pcm.c    pcm.c
                        printk("found  card->platform  %s \n", __func__);
			found = 1;
			break;
		}
	if (!found) {
	...
	for (i = 0; i < card->num_links; i++) {
		...
		list_for_each_entry(dai, &dai_list, list)
			if (card->dai_link[i].cpu_dai == dai) {
                        printk("found  card->dai_link[i].cpu_dai  %s \n", __func__);
				found = 1;
				break;
			}//add by panzidong
                        else {
                           printk("DAI  registered     %s     %s(%d)  %s \n",dai->name,__FILE__,__LINE__,__func__);

                            }
		if (!found) {
			dev_dbg(card->dev, "DAI %s not registered\n",
				card->dai_link[i].cpu_dai->name);
                        printk("DAI  %s  not registered    %s(%d)  %s \n",card->dai_link[i].cpu_dai->name,__FILE__,__LINE__,__func__);
			return;
		}
		...
		for (i = 0; i < card->num_links; i++) {
			...
			list_for_each_entry(dai, &dai_list, list)
				if (card->dai_link[i].codec_dai == dai) {
                                 printk("found  card->dai_link[i].codec_dai  %s \n", __func__);
					found = 1;
					break;
				}
			if (!found) {
...
static int soc_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		dev_err(&pdev->dev, "Failed to register card\n");
                printk("Failed to register  %s(%d)  %s  \n",__FILE__,__LINE__,__func__);
		return ret;
	}
 ...
 int snd_soc_register_card(struct snd_soc_card *card)
{
	...
	card->instantiated = 0;
        printk(" Note  delete  mutex_lock(&client_mutex);  <1>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
        printk("add  card_list %s  \n ",__func__);
	list_add(&card->list, &card_list);
	snd_soc_instantiate_cards();
	mutex_unlock(&client_mutex);
        printk(" Note delete  mutex_unlock(&client_mutex);  <1>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
        printk(" registere card %s  %s(%d)  %s\n", card->name,__FILE__,__LINE__,__func__);
...
static int snd_soc_unregister_card(struct snd_soc_card *card)
{
        printk("mutex_lock(&client_mutex);  <2>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
	list_del(&card->list);
	mutex_unlock(&client_mutex);
        printk("mutex_unlock(&client_mutex);  <2>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
int snd_soc_register_dai(struct snd_soc_dai *dai)
{
	...
	INIT_LIST_HEAD(&dai->list);
        printk("mutex_lock(&client_mutex);  <3>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
        printk("list_add(&dai->list, &dai_list); dai->name : %s     %s(%d)  %s \n",dai->name,__FILE__,__LINE__,__func__);
	list_add(&dai->list, &dai_list);
	snd_soc_instantiate_cards();
	mutex_unlock(&client_mutex);
        printk("mutex_unlock(&client_mutex);  <3>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
void snd_soc_unregister_dai(struct snd_soc_dai *dai)
{
        ...
        printk("mutex_lock(&client_mutex);  <4>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
	list_del(&dai->list);
	mutex_unlock(&client_mutex);
        printk("mutex_unlock(&client_mutex);  <4>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
int snd_soc_register_platform(struct snd_soc_platform *platform)
{
	...
	INIT_LIST_HEAD(&platform->list);
        ...
        printk("mutex_lock(&client_mutex);  <5>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
	list_add(&platform->list, &platform_list);
	snd_soc_instantiate_cards();
	mutex_unlock(&client_mutex);

        printk("mutex_unlock(&client_mutex);  <5>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
{      
        printk("mutex_lock(&client_mutex);  <6>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
	list_del(&platform->list);
	mutex_unlock(&client_mutex);
        printk("mutex_unlock(&client_mutex);  <6>   %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
int snd_soc_register_codec(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < codec->num_dai; i++) {
        ...
        printk(" mutex_lock(&clinet_mutex) <7>  %s(%d)  %s \n",__FILE__,__LINE__,__func__);
//	mutex_lock(&client_mutex);
        printk("list_add(&codec->list,&code_list) %s(%d) %s \n ",__FILE__,__LINE__,__func__);
	list_add(&codec->list, &codec_list);
//	snd_soc_instantiate_cards();
//	mutex_unlock(&client_mutex);
        
        printk("  mutex_unlock(&clinet_mutex) <7> %s(%d)  %s \n",__FILE__,__LINE__,__func__);
        printk("  %s(%d)  %s \n",__FILE__,__LINE__,__func__);
...
void snd_soc_unregister_codec(struct snd_soc_codec *codec)
{
        ...
        printk("  mutex_lock(&clinet_mutex) <8> %s(%d)  %s \n",__FILE__,__LINE__,__func__);
	mutex_lock(&client_mutex);
	list_del(&codec->list);
	mutex_unlock(&client_mutex);
        printk("  mutex_unlock(&clinet_mutex) <8> %s(%d)  %s \n",__FILE__,__LINE__,__func__);

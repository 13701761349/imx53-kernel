...
// add by panzidong
struct wm831x *g_wm831x;
EXPORT_SYMBOL_GPL(g_wm831x);
// end by panzidong
...
enum wm831x_parent {
	...
	WM8320 = 0x8320,
	WM8325 = 0x8325,
        // add by panzidong for power key;
        WM8326 = 0x8326
        // end by panzidong for power key;
};
...
static struct wm831x *wm831x_export=NULL;
static int wm831x_finish=0;
// add by panzidong
int  wm831x_on_pin_state(void){
      int ret;
      if(wm831x_finish==0)    
           return 1;

      if(wm831x_export == NULL)
           return 1;
      ret = wm831x_reg_read(wm831x_export,WM831X_ON_PIN_CONTROL);
 //     printk("wm831x_on_pin_state ret=0x%x--%d--\n",ret,__LINE__);
       if(ret & (1<<3))
          return 1;    //  not asserted  mean pwr_key not be pressed;
       else
          return 0;   //    asserted     mean pwr_key be pressed;
}
EXPORT_SYMBOL_GPL(wm831x_on_pin_state);


int battery_data_read(){
   ...
   return  wm831x_auxadc_read(wm831x_export,2);
}

EXPORT_SYMBOL_GPL(battery_data_read);
...
void wm831x_poweroff_system(void)
{
	...
	printk("wm831x_poweroff_system ret=0x%x--%d--\n",ret,__LINE__);
	ret &= ~(1 << 15);
	printk("wm831x_poweroff_system ret=0x%x--%d--\n",ret,__LINE__);
	wm831x_reg_write(wm831x_export, WM831X_POWER_STATE, ret);
	printk("wm831x_poweroff_system ret=0x%x--%d--\n",ret,__LINE__);
	ret = wm831x_reg_read(wm831x_export, WM831X_LDO7_ON_CONTROL);
	printk("wm831x_poweroff_system ret=0x%x--%d--\n",ret,__LINE__);
	ret &= ~(0x7 << 13);
	printk("wm831x_poweroff_system ret=0x%x--%d--\n",ret,__LINE__);
	ret = wm831x_reg_write(wm831x_export, WM831X_LDO7_ON_CONTROL,ret);
}
...
static int wm831x_device_init(struct wm831x *wm831x, unsigned long id, int irq)
{
	...
	case WM8325:
		...
		break;
        // add by panzidong for wm8326;
	case WM8326:
		parent = WM8326;
		wm831x->num_gpio = 12;
		dev_info(wm831x->dev, "WM8326 revision %c\n", 'A' + rev);
		break;
        // end by panzidong for wm8326;
	default:
...
//	return wm831x_device_init(wm831x, id->driver_data, i2c->irq);
	ret=wm831x_device_init(wm831x, id->driver_data, i2c->irq);
        wm831x_finish=1;
        return ret;
}

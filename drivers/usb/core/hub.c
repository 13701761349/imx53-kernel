...
#define DBG(...) 
#define DBGG(...) 
#define DBG2(...) printk("%s,%s,%d: ",__FILE__,__FUNCTION__,__LINE__);printk(__VA_ARGS__)
struct usb_hub {
...
void usb_disconnect(struct usb_device **pdev)
{
	 ...
	 */
	usb_set_device_state(udev, USB_STATE_NOTATTACHED);
	dev_info (&udev->dev, "%s,%s,%d, USB disconnect, address %d\n", __FILE__,__FUNCTION__,__LINE__,udev->devnum);
					...
					u16 portstatus, u16 portchange)
{
	...
	dev_dbg (hub_dev,
		"%s,%s,%d, port %d, status %04x, change %04x, %s\n",
		__FILE__,__FUNCTION__,__LINE__,
		port1, portstatus, portchange, portspeed (portstatus));

	//DBG2("portstatus=%x,portchange=%x\n",portstatus,portchange);
...
static void hub_events(void)
{
	...
	while (1) {
		...
		dev_dbg(hub_dev, "%s,%s,%d, state %d ports %d chg %04x evt %04x\n",
				__FILE__,__FUNCTION__,__LINE__,
...
//printk("\n\n");
//DBG2("----------------------hub->descriptor->bNbrPorts=%d,i=%d\n",hub->descriptor->bNbrPorts,i);

...
// add by panzidong for support LCD panel power off and keypad led off
struct power_dev {
 int count;
 struct timer_list s_timer;
};
static struct power_dev *power_devp; /*设备结构体指针*/

//suppend=0  -> mean LCD power on and keypad led on
//suppend =1 -> mean LCD power off and keypad led off
static int suppend = 0;
...
void  GPIO_FREE(){
   gpio_free(MX53_SMD_LEFT_LED);
   gpio_free(MX53_SMD_LCD3V3_EN);
   gpio_free(MX53_SMD_LCD5V_EN);
   gpio_request(MX53_SMD_LCD3V3_EN,"3V3");
   gpio_request(MX53_SMD_LCD5V_EN,"5V5");
   gpio_request(MX53_SMD_LEFT_LED,"LEFT_LED");
}

void  LCD_AND_KEYPAD_ON()
{
...
void LCD_AND_KEYPAD_OFF()
{
   gpio_set_value(MX53_SMD_LCD5V_EN,0); 
   gpio_set_value(MX53_SMD_LCD3V3_EN,0);
   gpio_set_value(MX53_SMD_LEFT_LED,0);
}


// return 0 mean Power KEY was Pressed
// return 1 mean Power KEY was not Pressed
 extern int wm831x_on_pin_state(void);
int ON_PIN_STATE=1;
EXPORT_SYMBOL_GPL(ON_PIN_STATE);

int Power_state(){
    return  ON_PIN_STATE;    
}

static void power_timer_handle(unsigned long arg)
{
...
static struct task_struct *Power_Kthread=NULL;
static int  power_kthread(void *data){
 
  static int power_state=0;
  while(!kthread_should_stop()){

    // power_state=Power_state();
     power_state=wm831x_on_pin_state();
     if(power_state==0)
      power_devp->count++;
     else
      power_devp->count=0;

    // printk("power_state=%d,count=%d\n",power_state,power_devp->count);
     if(power_devp->count == 3){
          printk("Power key Pressed 3s, so power down of pmic");
	  wm831x_poweroff_system();
      }
     msleep(1000);
 }
...
// add by panzidong  for support watchdog
struct second_dev
{
...
#define MX53_SMD_GPIO_4   (0*32+4)    /*GPIO_1_4*/
void feed_watchdog(int *up)
{    
}
...
{
  /*在这里需要增加喂狗，前提是需要给初始化相关的GPIO_1_4 */
  feed_watchdog(&WdogUp);
...
// end by panzidong  for support watchdog
...
static int __init mxcuart_init(void)
{
	int ret = 0;
         ...
         power_devp = kmalloc(sizeof(struct power_dev),GFP_KERNEL);
         if(!power_devp)
           {
            ret =  -ENOMEM;
            goto  fail_malloc;
       ...
       Power_Kthread=kthread_run(power_kthread,"power_ktheread","power_kthread");
...
{
        // add by panzidong for x7 support watchdog
         del_timer(&second_devp->s_timer);
         kfree(second_devp);     /*释放设备结构体内存*/
        // end by panzidong for x7 support watchdog
        
        // add by panzidong for LCD3V3_EN and LEFT_LED
        // del_timer(&power_devp->s_timer);
        // kfree(power_devp);
        // end by panzidong for LCD3V3_EN and LEFT_LED   

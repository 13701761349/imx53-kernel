 ...
 */
//static unsigned short KScanRate = (5 * HZ) / 1000;
static unsigned short KScanRate = (10 * HZ) / 1000;
//static unsigned short KScanRate = (20 * HZ) / 1000;
 ...
 */
// add by panzidong 
static ReportMatrix[25+1]={0};
static int SetZero=0;
...
static int mxc_kpp_scan_matrix(void)
{
	...
	memset(cur_rcmap, 0, kpp_dev.kpp_rows * sizeof(cur_rcmap[0]));
        // add by panzidong
 //       for(ExtStartTimes=0;StartTimes<ScanTimes;ExtStartTimes++){ 
	for (col = 0; col < kpp_dev.kpp_cols; col++) {	/* Col */

      //  printk("add by panzidong panzidong kpp_dev.kpp_cols=%d  col=%d\n",kpp_dev.kpp_cols,col);
	...
	//	udelay(2);
		...
		for (row = 0; row < kpp_dev.kpp_rows; row++) {	/* sample row */
			if (TEST_BIT(reg_val, row) == 0) {
                                for(StartTimes=0;StartTimes<300;StartTimes++){
                                    reg_val = __raw_readw(kpp_dev.base + KPDR);
                                       if (TEST_BIT(reg_val, row) == 0){
                                         scan[col*5+row]++;
                                        }else{
                                ...
                                // add by panzidong
                                //   scan[col*5+row]++;
                  if(scan[col*5+row] >=290 && row !=0 && row !=1 && col!=0 && col!=1 ){  
                              //    printk("row=%d,col=%d\n",row,col);
			...
			for (row = 0; row < kpp_dev.kpp_rows; row++) {
				for (col = 0; col < kpp_dev.kpp_cols; col++) {
					if ((press_scancode[row][col] != -1)) {
						...
						scancode =
						    press_scancode[row][col];
//                                                printk("StateFirstDown  input_event scancode=%d\n",scancode);
                                               // printk("  %s(%d)  %s  StateFirstDown  input_event scancode=%d\n",__FILE__,__LINE__,__func__,scancode);

...
#define MAX11801_MAX_XC		        800
//#define MAX11801_MAX_YC			0xfff
#define MAX11801_MAX_YC			480
...
unsigned long  calibration[7]={0, 1, 0, 0, 0, 1, 1};
EXPORT_SYMBOL(calibration);
...
//          printk("start calibration\n");
//        for(i=0;i<7;i++)
//        printk("calibration[%d]= %ld\n",i,calibration[i]);
//        printk("*x_orig = %d ,*y_orig=%d\n",*x_orig,*y_orig);
           
 //       tmp_x= calibration[0]*(*x_orig);
 //       printk("calibration[0]*(*x_orig)=%ld \n",tmp_x);

//        tmp_y= calibration[1]*(*y_orig);
//        printk("calibration[1]*(*y_orig)=%ld \n",tmp_y);
   
         
	x = calibration[0] * (*x_orig) + calibration[1] * (*y_orig) + calibration[2];
//	x = calibration[0]  + calibration[1] * (*x_orig) + calibration[2]*(*y_orig);
        
         
//        printk("calibration first x = %ld  \n",x);
        
	x /= calibration[6];

//        printk("calibration second x = %ld \n",x);
	if (x < 0)
		x = 0;

//        tmp_x= calibration[3]*(*x_orig);
//        printk("calibration[3]*(*x_orig)=%ld \n",tmp_x);

//        tmp_y= calibration[4]*(*y_orig);
//        printk("calibration[4]*(*y_orig)=%ld \n",tmp_y);

	y = calibration[3] * (*x_orig) + calibration[4] * (*y_orig) + calibration[5];
//	y = calibration[3] + calibration[4] * (*x_orig) + calibration[5]*(*y_orig);
       

//        printk("calibration first  = %ld \n",y);

	y /= calibration[6];
        
//        printk("calibration second  = %ld \n",y);
...
static void max11801_ts_phy_init(struct max11801_data *data)
{
	...
	max11801_write_reg(client, OP_MODE_CONF_REG, 0x26);
}
